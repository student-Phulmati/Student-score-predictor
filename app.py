...
def send_otp_email(receiver, otp, name="User"):
    try:
        ...
        msg["To"]      = receiver
        html = f"""
        <div style='font-family:Arial;background:#0d1b2a;color:white;padding:26px;border-radius:18px'>
...
# =====================================================
# PHOTO-TYPE IMMERSIVE CSS
# =====================================================
def apply_css():
    dark = st.session_state.theme == "dark"
    is_welcome = (not st.session_state.logged_in and st.session_state.auth_page == "welcome")

    # Multiple beautiful unsplash photos for different pages
    LIBRARY_BG   = "https://images.unsplash.com/photo-1521587760476-6c12a4b040da?auto=format&fit=crop&w=1920&q=80"
    STUDENT_BG   = "https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&w=1920&q=80"
    STUDY_BG     = "https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?auto=format&fit=crop&w=1920&q=80"
    CAMPUS_BG    = "https://images.unsplash.com/photo-1498243691581-b145c3f54a5a?auto=format&fit=crop&w=1920&q=80"

    current_bg = LIBRARY_BG if is_welcome else STUDENT_BG

    if dark:
        # Deep navy/teal dark photo theme
        overlay     = "linear-gradient(135deg, rgba(5,10,40,0.82) 0%, rgba(0,20,60,0.78) 40%, rgba(0,40,80,0.72) 100%)"
        card_bg     = "rgba(255,255,255,0.06)"
        card_hover  = "rgba(255,255,255,0.11)"
        card_border = "rgba(0,212,255,0.18)"
        text1       = "#e8f4ff"
        text2       = "#8ec8e8"
        textm       = "#5ab8d8"
        acc1        = "#00d4ff"
        acc2        = "#00ffcc"
        acc3        = "#0099bb"
        tb_bg       = "rgba(4,8,32,0.96)"
        tb_border   = "rgba(0,212,255,0.14)"
        tb_text     = "#e8f4ff"
        tb_role     = "#5ab8d8"
        nav_c       = "#00d4ff"
        nav_bg      = "rgba(0,212,255,0.12)"
        inp_bg      = "rgba(8,18,52,0.92)"
        inp_text    = "#e0f4ff"
        inp_border  = "rgba(0,212,255,0.30)"
        btn_grad    = "linear-gradient(135deg, #0a1f6e 0%, #0077b6 50%, #00d4ff 100%)"
        btn_hover   = "linear-gradient(135deg, #0077b6 0%, #00d4ff 50%, #00ffcc 100%)"
        shadow      = "0 8px 40px rgba(0,0,0,0.40)"
        score_color = "#00ffcc"
        metric_bg   = "rgba(0,212,255,0.08)"
        grid_color  = "rgba(0,212,255,0.08)"
    else:
        # Bright airy light photo theme
        overlay     = "linear-gradient(135deg, rgba(240,252,255,0.82) 0%, rgba(210,240,255,0.80) 50%, rgba(230,248,255,0.78) 100%)"
        card_bg     = "rgba(255,255,255,0.78)"
        card_hover  = "rgba(255,255,255,0.92)"
        card_border = "rgba(0,150,200,0.20)"
        text1       = "#02314e"
        text2       = "#024f7a"
        textm       = "#0077b6"
        acc1        = "#0077b6"
        acc2        = "#00b4d8"
        acc3        = "#0096c7"
        tb_bg       = "rgba(255,255,255,0.96)"
        tb_border   = "rgba(0,119,182,0.14)"
        tb_text     = "#02314e"
        tb_role     = "#0077b6"
        nav_c       = "#0077b6"
        nav_bg      = "rgba(0,119,182,0.10)"
        inp_bg      = "rgba(255,255,255,0.95)"
        inp_text    = "#02314e"
        inp_border  = "rgba(0,119,182,0.28)"
        btn_grad    = "linear-gradient(135deg, #023e8a 0%, #0077b6 50%, #00b4d8 100%)"
        btn_hover   = "linear-gradient(135deg, #0077b6 0%, #00b4d8 50%, #48cae4 100%)"
        shadow      = "0 8px 40px rgba(0,90,150,0.18)"
        score_color = "#0077b6"
        metric_bg   = "rgba(0,119,182,0.08)"
    ...
    * {{ font-family: 'DM Sans', sans-serif !important; box-sizing: border-box; }}
    h1, h2, h3, .hero-title, .dash-title, .page-title, .top-name {{
    ...
    /* ═══════════════════════════════════════
       PHOTO-OVERLAY TOPBAR — Glass Ribbon
    ═══════════════════════════════════════ */
    .topbar-shell {{
        width: 100%;
        background: {tb_bg};
    ...
    .top-profile {{ display: flex; align-items: center; gap: 10px; padding: 2px 0; }}
    .top-avatar {{
        width: 48px; height: 48px; border-radius: 50%;
        display: flex; align-items: center; justify-content: center;
        overflow: hidden; flex-shrink: 0;
        background: linear-gradient(135deg, #0a1f6e, #0077b6, #00d4ff);
        font-size: 1.4rem;
        box-shadow: 0 0 0 3px {acc1}44, 0 4px 20px rgba(0,212,255,0.30);
        border: 2px solid {acc1}66;
    }}
    .top-avatar img {{ width:100%; height:100%; object-fit:cover; border-radius:50%; }}
    .top-name {{ font-size: 0.96rem; font-weight: 800; color: {tb_text}; line-height: 1.2; letter-spacing: -0.2px; }}
    .top-role {{ font-size: 0.68rem; font-weight: 600; color: {tb_role}; letter-spacing: 0.5px; text-transform: uppercase; }}
    ...
    .theme-top-btn .stButton > button:hover {{
        ...
        box-shadow: 0 4px 16px {acc1}44 !important;
    }}
    .signout-top-btn .stButton > button {{
        ...
        background: {btn_grad} !important;
        box-shadow: 0 4px 16px {acc1}33 !important;
    }}
    ...
    div[data-testid="stSegmentedControl"] button[aria-pressed="true"] {{
        color: {nav_c} !important;
        background: {nav_bg} !important;
        border-bottom: 2px solid {nav_c} !important;
        border-radius: 8px 8px 0 0 !important;
    }}

    /* ═══════════════════════════════════════
       PHOTO BACKDROP CONTENT AREAS
    ═══════════════════════════════════════ */
    .dash-wrap {{
    ...
    /* Hero greeting — large bold text over photo */
    .dash-title {{
    ...
    /* ── PHOTO-STYLE METRIC CARDS ── */
    /* Each metric card = mini photo-card with frosted overlay */
    .metrics-row {{
        display: grid;
    ...
    .mc::before {{
        ...
        position: absolute;
        inset: 0;
        background: linear-gradient(135deg, {acc1}12 0%, transparent 70%);
        border-radius: 22px;
        pointer-events: none;
    }}
    .mc:hover {{
        transform: translateY(-5px) scale(1.02);
        box-shadow: 0 20px 60px rgba(0,0,0,0.30), 0 0 0 1px {acc1}44;
    }}
    .mc-icon {{
        font-size: 1.6rem;
        display: block;
    ...
    .chart-glass::before {{
        ...
        height: 2px;
        background: linear-gradient(90deg, {acc1}, {acc2}, transparent);
    ...
    .avatar-circle {{
        ...
        font-size: 2.2rem;
        box-shadow: 0 0 0 6px {acc1}22, 0 8px 30px rgba(0,0,0,0.22);
    }}
    .avatar-circle img {{ width:100%; height:100%; object-fit:cover; border-radius:50%; }}

    /* ── METRIC CARD (report page) ── */
    .metric-card {{
        background: {card_bg};
        border: 1px solid {card_border};
        box-shadow: {shadow};
    ...
    .stFormSubmitButton > button {{
        ...
        background: {btn_grad} !important;
        color: white !important;
        box-shadow: 0 6px 20px rgba(0,212,255,0.22) !important;
    ...
    .stFormSubmitButton > button:hover {{
        ...
        box-shadow: 0 14px 36px rgba(0,212,255,0.36) !important;
    ...
    .stPasswordInput input, textarea {{
        ...
        border: 1.5px solid {inp_border} !important;
        border-radius: 12px !important;
        font-weight: 500 !important;
        caret-color: {inp_text} !important;
    }}
    ...
    [data-baseweb="menu"] li:hover {{ background: rgba(0,212,255,0.15) !important; }}
    ...
    .stAlert {{ border-radius: 16px !important; backdrop-filter: blur(12px); }}
    .stDataFrame {{ border-radius: 16px; overflow: hidden; }}

    /* ═══════════════════════════════════════
       WELCOME PAGE — FULL PHOTO HERO
    ═══════════════════════════════════════ */
    .welcome-wrap {{
        min-height: 100vh;
        display: flex;
    ...
    .hero-photo-card::before {{
        ...
        top: 0; left: 0; right: 0;
        height: 3px;
        background: linear-gradient(90deg, {acc1}, {acc2}, {acc1});
    ...
    /* Feature cards — photo-style horizontal strip */
    .feat-strip {{
        display: grid;
    ...
    .feat-card {{
        background: {'rgba(255,255,255,0.07)' if dark else 'rgba(255,255,255,0.70)'};
        border: 1px solid {card_border};
        border-radius: 18px;
    ...
    .feat-card::after {{
        ...
        height: 2px;
        background: linear-gradient(90deg, transparent, {acc1}, transparent);
        opacity: 0;
        transition: opacity 0.20s;
    }}
    .feat-card:hover {{ transform: translateY(-5px); box-shadow: 0 20px 50px rgba(0,0,0,0.22); }}
    .feat-card:hover::after {{ opacity: 1; }}
    .feat-icon {{ font-size: 2rem; display: block; margin-bottom: 8px; }}
    .feat-title {{ font-size: 1.0rem; font-weight: 800; color: {text1}; margin: 0 0 6px 0; font-family: 'Syne', sans-serif; }}
    .feat-line {{ width: 32px; height: 3px; background: linear-gradient(90deg, {acc1}, {acc2}); border-radius: 99px; margin: 0 auto 8px auto; }}
    .feat-desc {{ font-size: 0.75rem; color: {text2}; font-weight: 500; line-height: 1.6; }}

    /* Stats strip — pill badges */
    .stats-strip {{
        display: flex;
        gap: 10px;
    ...
    .stat-num {{ font-size: 1.05rem; font-weight: 800; color: {text1}; font-family: 'Syne', sans-serif; }}
    .stat-lbl {{ font-size: 0.66rem; font-weight: 700; color: {textm}; text-transform: uppercase; letter-spacing: 0.8px; }}

    /* Used for row */
    .used-row {{ display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; margin: 0 0 20px 0; }}
    .used-item {{ text-align: center; padding: 4px 8px; }}
    .used-icon {{ font-size: 1.5rem; display: block; margin-bottom: 3px; }}
    .used-label {{ font-size: 0.62rem; font-weight: 800; color: {textm}; text-transform: uppercase; letter-spacing: 0.8px; }}

    .welcome-footer {{ text-align: center; font-size: 0.76rem; color: {textm}; padding: 8px 0 0 0; font-weight: 500; }}
    ...
    .auth-theme-btn .stButton > button {{
        ...
        background: {'rgba(8,18,52,0.88)' if dark else 'rgba(255,255,255,0.88)'} !important;
        border: 1.5px solid {card_border} !important; color: {text1} !important;
    ...
    /* Page title style */
    .page-title {{ font-size: 1.9rem; font-weight: 800; margin: 0 0 4px 0; color: {text1}; letter-spacing: -0.6px; font-family: 'Syne', sans-serif !important; }}
    .subtext {{ color: {text2}; font-size: 0.88rem; margin: 0 0 16px 0; font-weight: 500; }}

    /* Divider accent */
    .accent-divider {{
        height: 2px;
        background: linear-gradient(90deg, {acc1}66, {acc2}44, transparent);
        border: none;
        border-radius: 2px;
        margin: 4px 0 18px 0;
    }}
...
def simple_pdf_graph(scores):
    ...
    drawing.add(String(10, 145, "Score History Graph", fontSize=12, fillColor=colors.HexColor("#0077b6")))
...
def generate_pdf(username, user_data, score, inputs, recs):
    ...
    HS  = ParagraphStyle("H", parent=sty["Heading2"], fontSize=13, textColor=colors.white,
                         spaceAfter=0, fontName="Helvetica-Bold", backColor=colors.HexColor("#023e8a"),
    ...
    t.setStyle(TableStyle([("GRID",(0,0),(-1,-1),.5,colors.HexColor("#90e0ef")),
    ...
    tr   = Table([[Paragraph("<b>Predicted Score</b>",NS),Paragraph(f"{score}/100",NS)],
                 ...
                 colWidths=[2.2*inch,4.4*inch])
    tr.setStyle(TableStyle([("GRID",(0,0),(-1,-1),.5,colors.HexColor("#90e0ef")),
        ("BACKGROUND",(0,0),(0,-1),colors.HexColor("#e8f8fc")),
    ...
    ti.setStyle(TableStyle([("GRID",(0,0),(-1,-1),.5,colors.HexColor("#90e0ef")),
...
def cc():
    ...
    return {
        ...
        "accent": "#00ffcc" if dark else "#023e8a",
    }
...
def score_trend_chart(records):
    ...
    fig.add_trace(go.Scatter(
        ...
        textfont=dict(color=c["text"], size=11, family="Syne"),
    ...
    fig.update_layout(
        title=dict(text="📈 Score Trend Over Time", font=dict(color=c["text"], size=14, family="Syne")),
...
def radar_chart(inputs):
    ...
    fig.update_layout(
        title=dict(text="🕸️ Academic Profile Radar", font=dict(color=c["text"], size=14, family="Syne")),
        polar=dict(
...
def factor_bar_chart(inputs):
    ...
    fig.update_layout(
        title=dict(text="📊 Key Factors Breakdown", font=dict(color=c["text"], size=14, family="Syne")),
        ...
        <div class="hero-photo-card">
          ...
          <div class="feat-strip">
            <div class="feat-card">
              ...
              <div class="feat-title">Smart Graphs</div>
              <div class="feat-line"></div>
              <div class="feat-desc">Visualize academic trends with interactive, insightful charts</div>
            </div>
            <div class="feat-card">
              <span class="feat-icon">🔮</span>
              <div class="feat-title">AI Prediction</div>
              <div class="feat-line"></div>
              <div class="feat-desc">ML-powered score prediction — quick, accurate & personalized</div>
            </div>
            <div class="feat-card">
              <span class="feat-icon">📄</span>
              <div class="feat-title">PDF Reports</div>
              <div class="feat-line"></div>
              <div class="feat-desc">Download detailed report & share instantly on WhatsApp</div>
            </div>
          </div>
          ...
          <div style="margin-bottom:16px;">
            <div style="text-align:center;font-size:0.72rem;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;opacity:0.6;margin-bottom:10px;">Used For</div>
            <div class="used-row">
...
def home_page(user):
    ...
    name    = user.get("full_name", st.session_state.username)
    scores  = [r["score"] for r in records]
    greeting = "Good Morning" if datetime.now().hour < 12 else ("Good Evening" if datetime.now().hour >= 17 else "Good Afternoon")

    st.markdown(f"""
    <div class='dash-wrap'>
      <div class='dash-title'>👋 {greeting}, {name}!</div>
      <p class='dash-sub'>Your academic performance dashboard — all insights in one place.</p>
      <hr class="accent-divider"/>
      <div class='metrics-row'>
        <div class='mc'>
          <span class='mc-icon'>🎯</span>
          <div class='mc-val'>{len(records)}</div>
          <div class='mc-lbl'>Attempts</div>
        </div>
        <div class='mc'>
          <span class='mc-icon'>🏆</span>
          <div class='mc-val'>{max(scores) if scores else 0}</div>
          <div class='mc-lbl'>Best Score</div>
        </div>
        <div class='mc'>
          <span class='mc-icon'>📊</span>
          <div class='mc-val'>{int(np.mean(scores)) if scores else 0}</div>
          <div class='mc-lbl'>Average</div>
        </div>
        <div class='mc'>
          <span class='mc-icon'>🕐</span>
          <div class='mc-val'>{scores[-1] if scores else 0}</div>
          <div class='mc-lbl'>Last Score</div>
        </div>
      </div>
    </div>""", unsafe_allow_html=True)

    if records:
        st.markdown("<div style='padding:0 32px 24px 32px;'>", unsafe_allow_html=True)
...
def prediction_page(user):
    ...
    if submitted:
        ...
        record= {"date":datetime.now().strftime("%d-%m %H:%M"),"score":score,
        ...
        <div style='text-align:center;padding:24px 0 14px 0'>
          <div class='score-badge'>{score}<span style='font-size:1.2rem;opacity:0.7'>/100</span></div>
